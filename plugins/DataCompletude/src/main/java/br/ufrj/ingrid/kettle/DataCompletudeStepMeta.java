...
@InjectionSupported( localizationPrefix = "DataCompletudeStepMeta.Injection." )
public class DataCompletudeStepMeta extends BaseStepMeta implements StepMetaInterface {
   ...
   */
  private static final Class<?> PKG = DataCompletudeStepMeta.class; // for i18n purposes

  /**
   * Stores the name of the field added to the row-stream. 
   */
  private String inputDBpedia;
  private String inputTemplate;

  /**
   * Constructor should call super() to make sure the base class has a chance to initialize properly.
   */
  public DataCompletudeStepMeta() {
    super();
  }
  ...
  public StepDialogInterface getDialog( Shell shell, StepMetaInterface meta, TransMeta transMeta, String name ) {
    return new DataCompletudeStepDialog( shell, meta, transMeta, name );
  }
  ...
  public StepInterface getStep( StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, TransMeta transMeta,
      Trans disp ) {
    return new DataCompletudeStep( stepMeta, stepDataInterface, cnr, transMeta, disp );
  }
  ...
  public StepDataInterface getStepData() {
    return new DataCompletudeStepData();
  }
   ...
   * @param inputDBpedia the name of the field added
   * @param inputTemplate the name of the field added
   */
  public void setInputDBpedia( String inputDBpedia ) {
    this.inputDBpedia = inputDBpedia;
  }
  ...
  public String getXML() throws KettleValueException {
    ...
    xml.append( XMLHandler.addTagValue( "inputDBpedia", inputDBpedia ) );
    xml.append( XMLHandler.addTagValue( "inputTemplate", inputTemplate ) );
    return xml.toString();
  }
  ...
  public void loadXML( Node stepnode, List<DatabaseMeta> databases, IMetaStore metaStore ) throws KettleXMLException {
    try {
    	setInputDBpedia( XMLHandler.getNodeValue( XMLHandler.getSubNode( stepnode, "inputDBpedia" ) ) );
    	setInputTemplate( XMLHandler.getNodeValue( XMLHandler.getSubNode( stepnode, "inputTemplate" ) ) );
    } catch ( Exception e ) {
  ...
  public void saveRep( Repository rep, IMetaStore metaStore, ObjectId id_transformation, ObjectId id_step )
    ...
    try {
      rep.saveStepAttribute( id_transformation, id_step, "inputDBpedia", inputDBpedia ); //$NON-NLS-1$
      rep.saveStepAttribute( id_transformation, id_step, "inputTemplate", inputTemplate ); //$NON-NLS-1$
    } catch ( Exception e ) {
  ...
  public void readRep( Repository rep, IMetaStore metaStore, ObjectId id_step, List<DatabaseMeta> databases )
      throws KettleException {
    try {
      inputDBpedia  = rep.getStepAttributeString( id_step, "inputDBpedia" ); //$NON-NLS-1$
      inputTemplate  = rep.getStepAttributeString( id_step, "inputTemplate" ); //$NON-NLS-1$
    } catch ( Exception e ) {
  ...
  public void getFields( RowMetaInterface inputRowMeta, String name, RowMetaInterface[] info, StepMeta nextStep,
    ...
    ValueMetaInterface dbpedia = new ValueMetaString( inputDBpedia );

    // setting trim type to "both"
    dbpedia.setTrimType( ValueMetaInterface.TRIM_TYPE_BOTH );

    // the name of the step that adds this field
    dbpedia.setOrigin( name );

    // modify the row structure and add the field this step generates  
    inputRowMeta.addValueMeta( dbpedia );
    
    // a value meta object contains the meta data for a field
    ValueMetaInterface template = new ValueMetaString( inputTemplate );

    // setting trim type to "both"
    template.setTrimType( ValueMetaInterface.TRIM_TYPE_BOTH );

    // the name of the step that adds this field
    template.setOrigin( name );

    // modify the row structure and add the field this step generates  
    inputRowMeta.addValueMeta( template );
  }
